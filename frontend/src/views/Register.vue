...
const userRegister = async () => {
  ...
  try {
    ...
    const response = await auth.Register(`${apiRoot}/register`, user);
    ...
    <div class="bg-cover bg-center h-screen bg-opacity-50" :style="{ backgroundImage: `url(${Spacebg})` }">
        <div class="flex items-center justify-center h-full">
            <div
                ...
                <p class="p-2 pl-8 text-black font-smath text-3xl font-bold pb-[-200px]">Register</p>
                ...
                <p v-if="errorMessage" class="text-red-600 text-center mt-[-10px]">{{ errorMessage }}</p>
                <p v-if="successMessage" class="text-green-600 text-center mt-[-10px]">{{ successMessage }}</p>
                ...
                <div class="text-black font-smath flex items-center justify-center">
                    ...
                    <router-link to="/login" class="ml-2 text-black-600 underline hover:text-blue-800">Login here</router-link>
                </div>
            </div>
        </div>
    </div>
